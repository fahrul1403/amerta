...
<head>
    ...
    <title>LOGIN KADER | AMERTA</title>
...
<body>
    ...
    <div>
        ...
        <div class="w-full max-w-sm mx-auto mt-16 p-4 border border-gray-200 rounded-lg shadow sm:p-6 md:p-8">
            <form class="space-y-6" action="{{ route('login_kader') }}" method="POST">
                @csrf
                <h5 class="text-2xl text-primary font-bold dark:text-white text-center">Login Form Kader</h5>
                @if (session('error'))
                ...
                <div>
                    ...
                    <input type="password" name="password" id="no_str" placeholder="••••••••"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-4 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" />
                    @error('no_str')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="text-sm font-medium text-gray-500 dark:text-gray-300">
                    Not registered? <a href="{{ route('register', ['role' => 'kader']) }}"
